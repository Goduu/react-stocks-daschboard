...
import {
  ...
  getStocksPrices,
  test,
} from "../../../shared/functions/requests";
...
export default function SubscriptionTable(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        let res =
          (100 * (currentPrices[params.row.asset] - params.row.value)) /
          params.row.value;
        res = res.toFixed(2);
        return params.row.operation === BUY ? (
          res > 0 ? (
            ...
            <ColorfulChip label={`${res} %`} color={theme.palette.error.dark} />
          )
        ) : (
          ""
        );
  ...
  useEffect(() => {
    ...
    if (tickers.length > 0) {
      getStocksPrices(tickers, token).then((res) => {
